...
class BroHttpPyParserTestCase(unittest.TestCase):
    """Tests for BroHttpPyParser"""


    def test_returns_a_dictionary(self):

        returnedExample = bro_http_parser(fileName)
        self.assertTrue(dict == type(returnedExample))

    def test_known_return_string(self):
        pprint(bro_http_parser(exceptionFile))
        self.assertTrue(knownValues == bro_http_parser(fileName)['1'])
